...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch('/api/products?tag=bestSeller')
      .then((r) => r.json())
      .then((data) => setBestSellers((data.data || data).slice(0, 4)))
      .catch(() => {});

    fetch('/api/promotions')
      .then((r) => r.json())
      .then((data) => setPromotions((data.data || data).slice(0, 4)))
      .catch(() => {});

    fetch('/api/products/radiance-ritual-set')
      .then((r) => r.json())
      .then((data) => setBundleProduct(data.data || data))
      .catch(() => {});
  }, []);
